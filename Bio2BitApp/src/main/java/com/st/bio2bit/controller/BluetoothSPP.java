...
public class BluetoothSPP {
    ...
    private boolean isAndroid = com.st.bio2bit.model.BluetoothState.DEVICE_ANDROID;
    ...
    public void startService(boolean isAndroid) {
        if (mChatService != null) {
            if (mChatService.getState() == com.st.bio2bit.model.BluetoothState.STATE_NONE) {
    ...
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
            case com.st.bio2bit.model.BluetoothState.MESSAGE_WRITE:
                break;
            case com.st.bio2bit.model.BluetoothState.MESSAGE_READ:
                ...
                break;
            case com.st.bio2bit.model.BluetoothState.MESSAGE_DEVICE_NAME:
                mDeviceName = msg.getData().getString(com.st.bio2bit.model.BluetoothState.DEVICE_NAME);
                mDeviceAddress = msg.getData().getString(com.st.bio2bit.model.BluetoothState.DEVICE_ADDRESS);
                if(mBluetoothConnectionListener != null)
                    mBluetoothConnectionListener.onDeviceConnected(mDeviceName, mDeviceAddress);
                isConnected = true;
                break;
            case com.st.bio2bit.model.BluetoothState.MESSAGE_TOAST:
                Toast.makeText(mContext, msg.getData().getString(com.st.bio2bit.model.BluetoothState.TOAST)
                        , Toast.LENGTH_SHORT).show();
                break;
            case com.st.bio2bit.model.BluetoothState.MESSAGE_STATE_CHANGE:
                if(mBluetoothStateListener != null)
                    mBluetoothStateListener.onServiceStateChanged(msg.arg1);
                if(isConnected && msg.arg1 != com.st.bio2bit.model.BluetoothState.STATE_CONNECTED) {
                    if(mBluetoothConnectionListener != null)
                ...
                if(!isConnecting && msg.arg1 == com.st.bio2bit.model.BluetoothState.STATE_CONNECTING) {
                    isConnecting = true;
                } else if(isConnecting) {
                    if(msg.arg1 != com.st.bio2bit.model.BluetoothState.STATE_CONNECTED) {
                        if(mBluetoothConnectionListener != null)
    ...
    public void connect(Intent data) {
        String address = data.getExtras().getString(com.st.bio2bit.model.BluetoothState.EXTRA_DEVICE_ADDRESS);
    ...
    public void send(byte[] data, boolean CRLF) {
        if(mChatService.getState() == com.st.bio2bit.model.BluetoothState.STATE_CONNECTED) {
            if(CRLF) {
