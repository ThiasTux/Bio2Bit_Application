        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    if (BGWConst.WAIT_FOR_BYTE) {
                        if (BGWConst.WAIT_FOR_START && data == BGWConst.START_OF_PACKET) {
                            arr_byte.add(data);
                            BGWConst.WAIT_FOR_START = false;
                            BGWConst.WAIT_FOR_FRAME_CONTROL = true;
                        } else if (BGWConst.WAIT_FOR_FRAME_CONTROL && data == BGWConst.FRAME_CONTROL) {
                            arr_byte.add(data);
                            BGWConst.WAIT_FOR_FRAME_CONTROL = false;
                            BGWConst.WAIT_FOR_COMMAND = true;
                        } else if (BGWConst.WAIT_FOR_COMMAND && BGWConst.BGWCommandID.isBGWCommandId((byte) data)) {
                            arr_byte.add(data);
                            BGWConst.WAIT_FOR_COMMAND = false;
                            BGWConst.WAIT_FOR_CLUSTER = true;
                        } else if (BGWConst.WAIT_FOR_CLUSTER && BGWConst.BGWClusterID.isBGWCluster((byte) data)) {
                            arr_byte.add(data);
                            int clLen = 0;
                            if (data > 240) {
                                byte clusterLenght[] = new byte[2];
                                clusterLenght[0] = (byte) mmInStream.read();
                                clusterLenght[1] = (byte) mmInStream.read();
                                try {
                                    clLen = Utils.bytesToInt(Const.LITTLE_ENDIAN, clusterLenght);
                                } catch (Utils.ByteLenghtException e) {
                                    e.printStackTrace();
                                }
                            } else {
                                clLen = mmInStream.read();
                            }
                            for (int i = 0; i < clLen; i++) {
                                arr_byte.add(mmInStream.read());
                            }
                            BGWConst.WAIT_FOR_CLUSTER = true;
                            BGWConst.WAIT_FOR_END=true;
                        } else if(BGWConst.WAIT_FOR_END && data == BGWConst.END_OF_PACKET){
                            arr_byte.add(data);
                            ...
                            BGWConst.WAIT_FOR_END=false;
                            BGWConst.WAIT_FOR_CLUSTER=false;
                            BGWConst.WAIT_FOR_START=true;
                        }
                    }
                } catch (IOException e) {
                    connectionLost();
